...
### Чому є confirmation flow

Discord не дозволяє сховати кнопки публічного повідомлення тільки для одного користувача. Тому Worker робить так:

1. У публічному embed є кнопка підтвердження.
2. Користувач натискає її.
3. Worker відкриває приватне ephemeral повідомлення.
4. Фінальна кнопка прийняття/відмови є тільки в цьому приватному повідомленні.
5. Після дії Worker оновлює саме приватне повідомлення і прибирає кнопки тільки для цього користувача.

### Дії правил

- Прийняти правила: Worker видає одну або кілька ролей через Discord API.
- Відмовитися: Worker кикає користувача із сервера.
