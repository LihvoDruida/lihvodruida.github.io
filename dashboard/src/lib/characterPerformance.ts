...
function rioRoleScore(
  details: RaiderIoCharacterDetails,
  role: RaiderIoScoreSegmentKey,
) {
...
function normalizedRioScore(details: RaiderIoCharacterDetails) {
  const score =
    finiteNumber(details.snapshot?.currentScore) ??
    rioRoleScore(details, "all");
...
function normalizedRaidScore(summary: WarcraftLogsMetricSummary) {
  ...
  const recent = finiteNumber(
    summary.recentStats.averagePercentile ??
...
function signal(
  ...
  value: string,
  hint: string,
  tone: CharacterPerformanceSignal["tone"],
): CharacterPerformanceSignal {
...
export function buildCharacterPerformanceEcosystem(input: {
  ...
  const gearScore =
    finiteNumber(input.itemLevel) !== null
      ? clamp(((input.itemLevel as number) / 300) * 100)
      : null;
  ...
  const signals: CharacterPerformanceSignal[] = [
    ...
    signal(
      ...
      mythicPlusScore !== null && mythicPlusScore >= 70
        ? "good"
        : mythicPlusScore !== null && mythicPlusScore < 35
          ? "warn"
          : "neutral",
    ),
    signal(
