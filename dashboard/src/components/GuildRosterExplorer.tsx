...
function WarcraftLogsBadges({ member }: { member: GuildRosterMember }) {
    ...
    return (
      ...
      >
        <span>
          <small>WCL</small>
          <strong>—</strong>
          <em>{snapshot.status === "not_found" ? "немає" : "помилка"}</em>
        </span>
      </div>
    );
  }
  ...
  const visible = metrics.filter((item): item is NonNullable<typeof item> =>
    Boolean(item),
  );
    ...
    <div className="guild-wcl-badges" aria-label="Warcraft Logs HPS/DPS">
      {visible.map((metric) => (
        ...
        >
          ...
          <strong>{formatNumber(metric.max ?? metric.average ?? 0, 1)}</strong>
          <em>
